...
/// Article, about, and feed navigation links.
fn nav_links() -> Element(msg) {
  html.nav(
    [
      attribute.attribute("aria-label", "主要导航"),
      components.classes(["flex", "gap-4"]),
    ],
    [
      ...
      html.a([attribute.href(rss_url), nav_link_classes()], [
        element.text("RSS 订阅"),
      ]),
    ],
  )
...
fn theme_toggle() -> Element(msg) {
  html.button(
    [
      ...
      attribute.attribute("aria-label", "切换深色模式"),
      icon_link_classes(),
    ],
    [
...
fn socials() -> Element(msg) {
  element.fragment([
    icon_link(github_url, "访问 GitHub 主页", svg.feather_icon("github")),
    icon_link(mastodon_url, "访问 Mastodon 主页", svg.mastodon(24)),
    icon_link(x_url, "访问 X 主页", svg.x(24)),
    icon_link(linkedin_url, "访问 LinkedIn 主页", svg.feather_icon("linkedin")),
    icon_link(rss_url, "RSS 订阅", svg.feather_icon("rss")),
  ])
}

/// A link styled for icon display.
fn icon_link(href: String, label: String, icon: Element(msg)) -> Element(msg) {
  html.a(
    [
      attribute.href(href),
      attribute.target("_blank"),
...
const theme_toggle_js = "(function(){var d=document.documentElement;var isDark=d.classList.contains('dark');if(isDark){d.classList.remove('dark')}else{d.classList.add('dark')}try{window.localStorage.setItem('theme',isDark?'light':'dark')}catch(_){}})()"
